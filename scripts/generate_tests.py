...
# Print headers
# Auto‑generated test file header
print('// File autogenerated with /scripts/generate_tests.py')
print()
# In Rust 2018+ editions `extern crate` is unnecessary.
...
for (idx, title) in enumerate(sorted(data)):
    ...
    for template in (x[0] for x in data[title]['testcases']):
        # unwrap() because UriTemplate::new now returns a Result
        print('        UriTemplate::new("%s").unwrap(),' % template)
    print('    ];')
    print()
    ...
    for i in range(num_tests):
        ...
        if isinstance(answer, str):
            ...
            for s in answer:
                print('        "%s",' % s)
            print('    ];')
            print('    assert!(template_%d_answers.contains(&templates[%d].build().as_str()));' % (i, i))
        else:
